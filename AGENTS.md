...
- **Always test before committing.** Never commit (submodule or parent) until the affected module(s) build and their tests pass green. Run at least `mvn -B -pl <module> -am test` (or `verify`) for every module touched and only commit when it succeeds.
...
- Submodule `ignore = all`: `.gitmodules` sets `ignore = all` on every submodule, which makes `git status` hide gitlink drift AND makes plain `git add <path>` silently skip pointer updates. Use `git add -f <submodule>` to stage a new commit, and commit the parent pointer so CI checks out the intended SHA. `ignore = all` ALSO hides gitlink changes from `git diff` / `git show --stat` (only regular files show), so don't assume a parent commit missed a submodule pin just because it's absent from the stat — verify with `git ls-tree HEAD <submodule>` or `git --no-pager diff --ignore-submodules=none`.
...
- Mockito inline agent OOM on large classes in the test fork: in a memory-constrained sandbox, `mock(SomeLargeClass.class)` (mockito inline/ByteBuddy instrumenting a big class, e.g. `EbMSMessageProcessor` ~400 lines) crashes the surefire fork with `java.lang.instrument ASSERTION FAILED ... can't create name string` + `OutOfMemoryError` in `surefire-forkedjvm-*` threads — NOT an assertion failure. Mocking smaller classes is fine (e.g. `CPAManagerTest` passes). Prefer a real instance or mock an interface in such cases; the unit under test can be exercised without mocking the huge collaborator.
- EbMS SOAP faultstring is spec-mandated (`ebmsErrorReason`, per EbMS 2.0 §3.4.1.2) and is asserted by the ITs in `EbMSMessageServiceImplIT` — do not "sanitize" it to a generic string (breaks ~20 tests + the spec). The client-facing info-disclosure risk that does exist is full message BODIES being logged via the `nl.clockwork.ebms.message` logger (default INFO); body logging lives in `EbMSInputStreamHandler`. Also note `EbMSMessageDecrypter`/`SignatureValidator` pass raw crypto-exception `e.getMessage()` into `ebmsErrorReason` (residual disclosure).
